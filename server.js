...
// Login: máx 10 intentos por IP cada 15 min
const loginLimiter = rateLimit({
  ...
  legacyHeaders: false,
  handler: (req, res) => {
    console.warn(`[security] Demasiados intentos de login desde ${req.ip} a las ${new Date().toISOString()}`);
    res.status(429).json({ ok: false, error: 'Demasiados intentos. Esperá 15 minutos.' });
  },
});

// API general: máx 200 requests por IP cada 10 min
const apiLimiter = rateLimit({
  ...
  legacyHeaders: false,
  handler: (req, res) => {
    console.warn(`[security] Rate limit API superado desde ${req.ip}`);
    res.status(429).json({ error: 'Límite de requests superado. Esperá unos minutos.' });
  },
});
...
// ── JWT LIVIANO (sin dependencias externas) ───────────────────
...
function verifyToken(token) {
  try {
    ...
    if (Date.now() - payload.iat > 12 * 60 * 60 * 1000) return null; // expira 12hs
    return payload;
  } catch { return null; }
}
...
async function fetchClientMetrics(client) {
  ...
  const makeRange = (since, until) =>
    `/${adAccountId}/insights?time_range=${encodeURIComponent(JSON.stringify({ since: dateStr(since), until: dateStr(until) }))}&fields=${fields}&level=account`;
...
function calcHealthScore(client, metrics) {
  ...
  if (client.client_type === 'ecommerce') {
    const roasA = metrics.month?.roas || 0, roasT = client.target_roas || 0;
    if (roasT > 0 && roasA > 0) {
  ...
  if (client.client_type === 'leads') {
    const cplA = metrics.month?.cpl || 0, cplT = client.target_cpl || 0;
    if (cplT > 0 && cplA > 0) {
...
function auth(req, res, next) {
  // Acepta JWT en header Authorization: Bearer <token>
  ...
  // Fallback: password legacy para compatibilidad
...
app.post('/api/login', loginLimiter, (req, res) => {
  ...
  const match = pwdBuffer.length === adminBuffer.length &&
    crypto.timingSafeEqual(pwdBuffer, adminBuffer);
  ...
  if (match) {
    ...
    console.warn(`[security] Intento de login fallido desde ${req.ip} a las ${new Date().toISOString()}`);
